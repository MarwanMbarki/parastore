			...
			<div class="top-infobox text-right col-xs-6">
				<?php if ( class_exists( 'WooCommerce' ) && get_theme_mod( 'my-account-link', 1 ) == 1 ) { // Login Register   ?>
					<?php if ( is_user_logged_in() ) { ?>
						<a class="my-account-link" href="<?php echo esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ); ?>" title="<?php esc_attr_e( 'My Account', 'universal-store' ); ?>"><?php esc_html_e( 'My Account', 'universal-store' ); ?></a>
					<?php } else { ?>
						<a class="my-account-link" href="<?php echo esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ); ?>" title="<?php esc_attr_e( 'Login / Register', 'universal-store' ); ?>"><?php esc_html_e( 'Login / Register', 'universal-store' ); ?></a>
					<?php } ?> 
				<?php } ?>
					...
					<div class="header-search-form col-md-12">
						<form role="search" method="get" action="<?php echo get_permalink( woocommerce_get_page_id( 'shop' ) ); ?>">
							<select class="col-sm-4 col-xs-4" name="product_cat">
								<option value=""><?php esc_html_e( 'All', 'universal-store' ); ?></option> 
								<?php
							...
							<input class="col-sm-8 col-xs-8" name="s" type="text" placeholder="<?php esc_attr_e( 'Search for products', 'universal-store' ); ?>"/>
							<button type="submit"><?php esc_html_e( 'Go', 'universal-store' ); ?></button>
						</form>
					</div>
				</div>
			<?php endif; ?>
		...
		<nav id="site-navigation" class="navbar navbar-inverse" role="navigation">
			...
			<div class="navbar-header">
				<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-1-collapse">
					<span class="sr-only"><?php esc_html_e( 'Toggle navigation', 'universal-store' ); ?></span>
